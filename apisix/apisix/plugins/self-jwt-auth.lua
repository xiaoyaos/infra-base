...
local plugin_name = "self-jwt-auth"
local pcall = pcall
...
function _M.rewrite(conf, ctx)
    -- 如果命中白名单，则不做 jwt 校验 begin
    core.log.warn("uri: ", ctx.var.request_uri)
    -- core.log.warn("whitelist rules: ", conf.whitelist)

    if conf.whitelist then
        if not conf.whitelist_concat then
            local whitelist = {}
            for i, re_rule in ipairs(conf.whitelist) do
                whitelist[i] = re_rule
            end

            conf.whitelist_concat = core.table.concat(whitelist, "|")
            if conf.case_insensitive then
                conf.whitelist_concat = "(?i)" .. conf.whitelist_concat
            end
            core.log.info("concat whitelist: ", conf.whitelist_concat)
        end

        local from = re_find(ctx.var.request_uri, conf.whitelist_concat, "jo")
        if from then
            return
        end
    end
    -- 如果命中白名单，则不做 jwt 校验 end
    ...
    local user_key = (jwt_obj.payload and jwt_obj.payload.key) or conf.user_key
    if not user_key then
        core.log.warn(jwt_obj.payload.name)
        -- core.log.warn(jwt_obj.payload)
    ...
    core.log.warn("jwt object: ", core.json.delay_encode(jwt_obj))
